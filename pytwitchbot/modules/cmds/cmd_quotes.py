...
class CmdModuleQuotes(CmdModule):
    ...
    def del_quote(self, userinfo, dest, args):
        if self.irc.perms.check_perm(userinfo[0], 1):
            if len(args) > 1 and args[1] != '':
                self.irc.sql.sql_query('DELETE FROM py_quotes WHERE qchid=? AND qchan=?', (int(args[1]), dest,))
    ...
    def read_quote(self, userinfo, dest, args):
        if len(args) > 1 and args[1] != '':
            try:
                self.irc.sql.sql_query('SELECT * FROM py_quotes WHERE qchid=? AND qchan=?', (int(args[1]), dest,))
    ...
    def count_quotes(self, userinfo, dest, args):
        self.irc.sql.sql_query('SELECT COUNT(*) FROM py_quotes WHERE qchan=?', (dest,))
