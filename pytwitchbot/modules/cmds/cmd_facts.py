...
class CmdModuleFacts(CmdModule):
    ...
    def add_fact(self, userinfo, dest, args):
        if len(args) > 2 and args[1] != '' and args[2] != '':
            try:
                facttext = self.irc.sql.escape(" ".join(args[2:]))
                date = self.log.get_local_time
                self.irc.sql.sql_query('INSERT INTO py_facts VALUES (%s, \'%s\', 0, \'%s\', \'%s\', %s)' % (
                    self.irc.sql.escape(args[1]), userinfo[0],
                    date, dest, facttext))
                self.irc.msg(
                    dest, 'Fact %s has been added to the database.' % (args[1]))
            except Exception as err:
                self.log.output('Error while adding fact to the database: %s' % err)
                self.irc.msg(dest, 'That fact already exists!')
        else:
    ...
    def del_fact(self, userinfo, dest, args):
        if self.irc.perms.check_perm(userinfo[0], 1):
            if len(args) > 1 and args[1] != '':
                self.irc.sql.sql_query('DELETE FROM py_facts WHERE factname=%s' % (
                    self.irc.sql.escape(args[1])))
    ...
    def read_fact(self, userinfo, dest, args):
        if len(args) > 1 and args[1] != '':
            try:
                self.irc.sql.sql_query('SELECT * FROM py_facts WHERE factname=%s' % (
                    self.irc.sql.escape(args[1])))
                factname, factauthor, lockstatus, date, channel, facttext = self.irc.sql.fetch(
                )
                self.irc.msg(dest, 'Fact %s by %s: %s' % (
                    factname, factauthor, facttext))
            except TypeError:
    ...
    def lock_fact(self, userinfo, dest, args):
        if self.irc.perms.check_perm(userinfo[0], 1):
            if len(args) > 1 and args[1] != '':
                self.irc.sql.sql_query('UPDATE py_facts SET factlock=1 WHERE factname=%s' % (
                    self.irc.sql.escape(args[1])))
    ...
    def unlock_fact(self, userinfo, dest, args):
        if self.irc.perms.check_perm(userinfo[0], 1):
            if len(args) > 1 and args[1] != '':
                self.irc.sql.sql_query('UPDATE py_facts SET factlock=0 WHERE factname=%s' % (
                    self.irc.sql.escape(args[1])))
    ...
    def change_fact(self, userinfo, dest, args):
        if len(args) > 2 and args[1] != '' and args[2] != '':
            try:
                ...
                newtext = " ".join(args[2:])
                if lockstatus == 0:
                    self.irc.sql.sql_query('UPDATE py_facts SET fact=%s, factauthor=\'%s\' WHERE factname=%s' % (
                        self.irc.sql.escape(newtext), userinfo[0], self.irc.sql.escape(args[1])))
                    self.irc.msg(dest, 'Fact has been changed.')
                else:
                    self.irc.msg(
                        dest, 'The fact you\'re trying to change is locked.')
            except TypeError:
                self.irc.msg(dest, 'That fact doesn\'t exist!')
        else:
            self.irc.msg(dest, self.cmd_dict[args[0]]['help'])
